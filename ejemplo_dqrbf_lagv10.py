...
#ejemplo differential quadrature RBF para DUAL LAG Pennes equation CV RBF
...
L=5.0e-2
H=5.0e-2
nl=20  #number of nodes in each direction
nh=20
dx=L/(nl-1)
dy=H/(nh-1)
...
#inicializacion
for i in range(ntot):
    Tj1[i]=Tj0[i]+dt**2/(2*rho*cp*tauq)*Qm
...
for i in f3:#superior
    ...
    dv=Asouth*dy/2
    for m in range(np.size(st)):
        A[i,st[m]]=k*(1+taut/dt)*Asouth*dzsouth[m]\
        +k*(1+taut/dt)*Awest*dzwest[m]-k*(1+taut/dt)*Aeast*dzeast[m]
    A[i,i]+=rho*cp*tauq/dt**2*dv+\
    (rho*cp+wb*rhob*cpb*tauq)/dt*dv+wb*rhob*cpb*dv
dst_f4=[]
st_f4=[]
...
for i in f9:#internos
    ...
    dst_f9.append(lap)
    for m in range(np.size(st)):
        A[i,st[m]]=-k*(1+taut/dt)*lap[m]
    A[i,i]+=rho*cp*tauq/dt**2+(rho*cp+wb*rhob*cpb*tauq)/dt+\
        wb*rhob*cpb
...
#        dz=dfsten(x,xs)
        ...
        B[i]=-k*(1-taut/dt)*np.dot(dz[1],T1st)
    for i in f2:#derecha
        x=np.array(xn[i])
        st=st_f2[f2.index(i)]
        xs=np.array([xn[m] for m in st])
#        dr=dfsten(x,xs)
        ...
        T0st=np.array([Tj0[m] for m in st])
        T1st=np.array([Tj1[m] for m in st])        
        B[i]=k*(1-taut/dt)*np.dot(dr[0],T1st)
    for i in f3:#superior
        x=np.array(xn[i])
        st=st_f3[f3.index(i)]
        xs=np.array([xn[m] for m in st])
        ...
        T0st=np.array([Tj0[m] for m in st] )
        T1st=np.array([Tj1[m] for m in st])        
        B[i]=(2*rho*cp*tauq/dt**2*Tj1[i]+\
        wb*rhob*cpb*Tb+(rho*cp+wb*rhob*cpb*tauq)/dt*Tj1[i]\
        -rho*cp*tauq/dt**2*Tj0[i]+Qm+Qsource(t))*dv\
        +k*(taut/dt)*Asouth*np.dot(dzsouth,T1st)\
        +k*(taut/dt)*Awest*np.dot(dzwest,T1st)\
        -k*(taut/dt)*Aeast*np.dot(dzeast,T1st)\
        -qp(x[0],t)*Anorth-tauq/dt*(qp(x[0],t+dt)-qp(x[0],t))*Anorth
    for i in f4:#izquierda
        x=np.array(xn[i])
        st=st_f4[f4.index(i)]
        xs=np.array([xn[m] for m in st])
...
#        lap=d2fsten(x,xs)[0]+1/x[0]*dfsten(x,xs)[0]+d2fsten(x,xs)[2]
        ...
        T1st=np.array([Tj1[m] for m in st])        
        B[i]=2*rho*cp*tauq/dt**2*Tj1[i]+\
        wb*rhob*cpb*Tb+(rho*cp+wb*rhob*cpb*tauq)/dt*Tj1[i]-rho*cp*tauq/dt**2*Tj0[i]\
        -k*(taut/dt)*np.dot(lap,T1st)    
        +Qm+Qsource(t)    
